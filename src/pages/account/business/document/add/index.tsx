...
const index = () => {
  ...
  const handleFrontImgUpload = async() =>{
    try {
     const apiRes = await henceforthApi.host.hostDocumentImgUplaod(frontImg);
      return apiRes.data;
    } catch (error) {
  ...
  const handBackImgUpload = async() =>{
    try {
     const apiRes = await henceforthApi.host.hostDocumentImgUplaod(backImg);
      return apiRes.data;
    } catch (error) {
      console.log("error in uplaodig document img", error)
    }
  ...
  const handleDocumentUpload = async(values:any) => {
   ...
   if(values.document_type == 1){
    ...
    try {
      ...
      const apiRes = await henceforthApi.host.uploadHostDocuments(payload);
